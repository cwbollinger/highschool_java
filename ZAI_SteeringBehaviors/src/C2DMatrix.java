...
class C2DMatrix {
	...
	public void TransformVector2Ds(Vector<Vector2D> vPoint) {
		for (int i = 0; i < vPoint.size(); ++i) {
			double tempX = (m_Matrix._11 * vPoint.get(i).x)
					+ (m_Matrix._21 * vPoint.get(i).y) + (m_Matrix._31);

			double tempY = (m_Matrix._12 * vPoint.get(i).x)
					+ (m_Matrix._22 * vPoint.get(i).y) + (m_Matrix._32);
